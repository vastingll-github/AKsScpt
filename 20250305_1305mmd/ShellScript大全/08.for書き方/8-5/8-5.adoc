...
* link:#shells[シェルの種類]
* link:#prompt[プロンプト]
* link:#output[メッセージ出力(echo&#44; printf)]
* link:#meta[メタ文字]
* link:#variables[シェル変数と環境変数($XXX)]
* link:#special[特殊変数($x)]
* link:#expansion[変数展開($\{...})]
* link:#readonly[読み込み専用(readonly)]
* link:#shell-script[シェルスクリプト(*.sh)]
* link:#comment[コメント(#)]
* link:#continue-line[継続行(\)]
* link:#command-terminator[コマンド区切り(;)]
* link:#group-command[グループコマンド(\{...})]
* link:#sub-shell[サブシェル((...))]
* link:#command-replace[コマンド置換($(...)&#44; `...`)]
* link:#init-file[初期化ファイル(.bash_profile/.bashrc)]
* link:#in-out-redirect[入出力・リダイレクト(>)]
* link:#here-document[ヒアドキュメント(<<)]
* link:#pipe[パイプ(|)]
* link:#background[バックグラウンド実行(&)]
* link:#history[ヒストリ(history)]
* link:#line-edit[行編集(Ctrl-a&#44; Ctrl-b&#44; ...)]
* link:#dirstack[ディレクトリスタック(dirs)]
* link:#condition[条件実行(; && ||)]
* link:#if-statemant[条件分岐(if)]
* link:#case[条件分岐(case)]
* link:#for[繰り返し(for)]
* link:#while[繰り返し(while)]
* link:#until[繰り返し(until)]
* link:#select[選択肢(select)]
* link:#break[ループ終了(break)]
* link:#continue[ループ継続(continue)]
* link:#read[入力(read)]
* link:#exit[シェルの終了(exit)]
* link:#trap[トラップ(trap)]
* link:#exec[シェル実行(exec)]
* link:#shift[引数シフト(shift)]
* link:#getopts[オプション解釈(getopts)]
* link:#source[ソース読み込み(source)]
* link:#alias[エイリアス(alias)]
* link:#bind[バインド(bind)]
* link:#jobs[ジョブ制御(Ctrl-Z&#44; jobs&#44; fg&#44; bg)]
* link:#function[関数(function)]
* link:#braceexpand[ブレース展開(\{...})]
* link:#expr[数値計算($((...)))]
* link:#array[配列((...)&#44; [...])]
* link:#unicode[Unicode文字($'\Uxxxx')]
* link:#options[シェルオプション(set)]
* link:#links[リンク]
...
* *ksh* : sh を改良したものです。ベル研のデビット・コーン氏が開発。csh
の機能も一部取り入れています。
...
プロンプトの色を変更するには *\e[太さ;色番号m*、元に戻すには *\e[m*
を、*\[* ... *\]* で挟んで指定します。太さは *0* が通常、*1*
が太字、色は、黒(30)、赤(31)、緑(32)、黄色(33)、青(34)、マジェンダ(35)、シアン(36)、灰色(37)、白(97)
などを指定できます。(→
https://en.wikipedia.org/wiki/ANSI_escape_code#Colors[詳細])
...
*pushd* _dir_ で移動し、戻る時は *pupd*、スタックがいくつかある時は
*pushd +*_num_
で移動します。__num__は0から数えます。スタックの0番目が常にカレントディレクトリとなります。
...
*\{*_n_*..*_m_*}*
は、__n__～__m__までの数値のリストを返却します。下記の例は 1～10
のループを回します。詳細は link:#braceexpand[ブレース展開]
を参照してください。
...
*fg* は指定したジョブをフォアグランドジョブとして再開します。ジョブは bg
と同様に指定します。
...
で囲まれた文字を展開できる機能があります。link:#for[for文]
...
* https://linuxjm.osdn.jp/html/GNU_bash/man1/bash.1.html[Bashマニュアル(日本語)]
...
[[div-gpt-ad-1737079436338-0]]

[[div-gpt-ad-1737079483388-0]]
